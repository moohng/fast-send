...
export default function App() {
  ...
  const uploadFile = (file: File | Blob, originalName?: string) => {
    ...
    setItems(p => [...p, { id: tempId, type: "file", originalName: name, size: (file.size / 1024 / 1024).toFixed(2) + " MB", time: new Date().toLocaleTimeString(), fullTime: new Date().toISOString(), senderId: CLIENT_ID, progress: 0 }]);
    ...
    xhr.onload = () => { if (xhr.status !== 200) { showToast('上传失败', 'error'); setItems(p => p.filter(x => x.id !== tempId)); } else { showToast('上传成功'); setItems(p => p.filter(x => x.id !== tempId)); } };
    xhr.send(formData);
  };
    ...
    <div className="h-screen bg-slate-50 flex flex-col font-sans text-slate-900 selection:bg-blue-100 selection:text-blue-700 overflow-hidden relative" onDragEnter={handleDragEnter} onDragOver={e => e.preventDefault()} onDragLeave={handleDragLeave} onDrop={handleDrop}>
      ...
      <div className="bg-white/80 backdrop-blur-md border-b shrink-0 z-50 px-4 py-3 flex items-center justify-between shadow-sm">
        ...
        <div className="flex gap-2">
          <button onClick={() => setShowQR(true)} className="p-2.5 bg-slate-100 hover:bg-blue-50 text-slate-600 hover:text-blue-600 rounded-xl transition-all"><QrCode size={20} /></button>
          {isElectron && <button onClick={() => setShowSettings(true)} className="p-2.5 bg-slate-100 hover:bg-blue-50 text-slate-600 hover:text-blue-600 rounded-xl transition-all"><Settings size={20} /></button>}
        </div>
      </div>
      <div onScroll={() => activeMenu && setActiveMenu(null)} className="flex-1 overflow-y-auto p-4 sm:p-6 w-full">
        <div className="max-w-full mx-auto flex flex-col gap-4">
          {items.map(item => (
            <MessageItem 
              key={item.id} 
              item={item} 
              isMe={item.senderId === CLIENT_ID || item.senderId === 'DESKTOP' || item.senderId === 'CLIPBOARD_SYNC' || item.senderId === 'CLIPBOARD_IMAGE'} 
              baseUrl={baseUrl}
              ...
              isElectron={isElectron} // 传入环境变量
            />
          ))}
          {items.length === 0 && (<div className="py-20 flex flex-col items-center justify-center text-slate-300"><UploadCloud size={64} strokeWidth={1} /><p className="mt-4 text-sm font-medium">暂无共享内容，开始发送吧</p></div>)}
