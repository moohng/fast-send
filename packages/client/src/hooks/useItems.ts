...
export const useItems = (baseUrl: string, socket: Socket | null, clientId: string, showToast: (m: string, t?: any) => void) => {
  ...
  // 移动端自动同步逻辑
  const autoDownloadForMobile = useCallback(async (item: SharedItem) => {
    ...
    try {
      console.log('[Sync] Auto downloading:', item.originalName);
      ...
      reader.onloadend = async () => {
        ...
        try {
          const isMedia = item.originalName.match(/\.(jpg|jpeg|png|gif|mp4|mov|webm)$/i);
          ...
          showToast(isMedia ? '媒体已保存到相册' : `文件已同步: ${item.originalName}`);
        } catch (e: any) {
          // 兜底：如果 ExternalStorage 不可用，尝试 Documents
          await Filesystem.writeFile({
            path: `FastSend/${item.originalName}`,
            data: base64Content,
