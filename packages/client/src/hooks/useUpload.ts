...
export const useUpload = (baseUrl: string, clientId: string, setItems: React.Dispatch<React.SetStateAction<SharedItem[]>>, showToast: (m: string, t?: any) => void) => {
  ...
  const uploadFile = useCallback(async (file: File) => {
    ...
    try {
      ...
      const mergeRes = await fetch(`${baseUrl}/api/upload/merge`, {
        ...
        body: JSON.stringify({ hash, fileName: file.name, total: totalChunks, senderId: clientId })
      });
